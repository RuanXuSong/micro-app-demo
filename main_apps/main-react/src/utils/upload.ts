...
// 最大文件大小限制 50M
export const ATTACHMENT_MAX_FILE_SIZE = 50 * BASE_BYTE;

// 最大文件大小限制 500M
export const ATTACHMENT_MAX_VIDEO_SIZE = 500 * BASE_BYTE;
...
export const isPermitFile = (
  ...
  const str = url || name;
  // 文件后缀
  const fileSuffix = '.' + string.getLastSubstring(str, '.');
    ...
    validator: (_rule: unknown, values: UploadFile[], callback: (error?: string) => void) => {
      if (values && values.length) {
        const validationFailedList = values.filter(
          (file) => file?.size && file.size / BASE_BYTE > maxSize,
        );
