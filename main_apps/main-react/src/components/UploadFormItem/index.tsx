 ...
 * @LastEditors: 阮旭松
 * @LastEditTime: 2022-07-26 14:47:43
 */
...
const CustomUpload: React.FC<CustomUpload> = forwardRef<any, CustomUpload>(
...
const UploadFormItem: React.FC<UploadFormItemProps> = (uploadItemProps) => {
  ...
  /** 判断是否可以上传 */
  const getBeforeUpload = (validatorObj: FileValidatorsProps) => (file: RcFile) => {
  ...
  return (
    <Form.Item
      label={renderLabel()}
      name={name}
      className={classNames(styles.uploadItemWrap, className)}
      style={style}
