...
export default function preFetch(apps: prefetchParamList): void {
  ...
  requestIdleCallback(() => {
    ...
    if (isArray(apps)) {
      apps.reduce(
        (pre, next) => pre.then(() => preFetchInSerial(next)),
        Promise.resolve()
      );
    }
  });
      ...
      if (isPlainObject(prefetchApp) && navigator.onLine) {
        ...
        if (
          ...
          const app = new CreateApp({
            ...
            url: prefetchApp.url,
            scopecss: !(
              prefetchApp.disableScopecss ?? microApp.disableScopecss
            ),
            useSandbox: !(
              prefetchApp.disableSandbox ?? microApp.disableSandbox
            ),
          });
...
function fetchGlobalResources(
  ...
  if (isArray(resources)) {
    const effectiveResource = resources!.filter(
      (path) =>
        isString(path) && path.includes(`.${suffix}`) && !cache.has(path)
    );

    const fetchResourcePromise = effectiveResource.map((path) =>
      fetchSource(path)
    );
