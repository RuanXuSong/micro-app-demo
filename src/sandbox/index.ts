...
export default class SandBox implements SandBoxInterface {
  ...
  start(baseRoute: string): void {
    if (!this.active) {
      ...
      this.microAppWindow.__MICRO_APP_BASE_ROUTE__ =
        this.microAppWindow.__MICRO_APP_BASE_URL__ = baseRoute;
  ...
  recordUmdSnapshot(): void {
    ...
    this.injectedKeys.forEach((key: PropertyKey) => {
      this.recordUmdInjectedValues!.set(
        key,
        Reflect.get(this.microAppWindow, key)
      );
    });
  }
  ...
  rebuildUmdSnapshot(): void {
    this.recordUmdInjectedValues!.forEach(
      (value: unknown, key: PropertyKey) => {
      ...
      for (const plugin of plugins) {
        if (isPlainObject(plugin)) {
          if (isArray(plugin.scopeProperties)) {
            this.scopeProperties = this.scopeProperties.concat(
              plugin.scopeProperties!
            );
          }
          if (isArray(plugin.escapeProperties)) {
            this.escapeProperties = this.escapeProperties.concat(
              plugin.escapeProperties!
            );
          }
        }
      }
  ...
  private initMicroAppWindow(
    ...
    microAppWindow.microApp = Object.assign(
      new EventCenterForMicroApp(appName),
      {
        removeDomScope,
        pureCreateElement,
      }
    ...
    microAppWindow.hasOwnProperty = (key: PropertyKey) =>
  ...
  private setMappingPropertiesWithRawDescriptor(
    microAppWindow: microAppWindowType
  ): void {
  ...
  private createDescriptorForMicroAppWindow(
    key: PropertyKey,
    value: unknown
  ): PropertyDescriptor {
    const {
      ...
      writable,
      set,
    } = Object.getOwnPropertyDescriptor(globalEnv.rawWindow, key) || {
      writable: true,
    };
