...
export default class CreateApp implements AppInterface {
        ...
        if (!this.umdMode) {
          ...
          if (isFunction(mount) && isFunction(unmount)) {
            ...
            try {
              ...
              logError(
                "an error occurred in the mount function \n",
                this.name,
                e
              );
            }
  ...
  private getUmdLibraryHooks(): Record<string, unknown> {
    ...
    if (appStates.UNMOUNT !== this.state) {
      ...
      this.libraryName =
        getRootContainer(this.container!).getAttribute("library") ||
        `micro-app-${this.name}`;
      // do not use isObject
      return typeof global[this.libraryName] === "object"
        ? global[this.libraryName]
        : {};
    }
