...
class EventCenterForGlobal {
  ...
  addGlobalDataListener(
    cb: CallableFunctionForInteract,
    autoTrigger?: boolean
  ): void {
...
export class EventCenterForBaseApp extends EventCenterForGlobal {
  ...
  addDataListener(
    ...
    eventCenter.on(
      formatEventName(formatAppName(appName), false),
      cb,
      autoTrigger
    );
  }
  ...
  removeDataListener(appName: string, cb: CallableFunction): void {
    isFunction(cb) &&
      eventCenter.off(formatEventName(formatAppName(appName), false), cb);
  }
  ...
  getData(
    ...
    return eventCenter.getData(
      formatEventName(formatAppName(appName), fromBaseApp)
    );
  }
...
export function recordDataCenterSnapshot(
  microAppEventCenter: EventCenterForMicroApp
): void {
  ...
  const subAppEventInfo = eventCenter.eventList.get(
    formatEventName(appName, true)
  );
  if (subAppEventInfo) {
    microAppEventCenter.umdDataListeners.normal = new Set(
      subAppEventInfo.callbacks
    );
  }
}
