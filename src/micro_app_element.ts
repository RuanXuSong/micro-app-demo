  ...
  class MicroAppElement extends HTMLElement implements MicroAppElementType {
    ...
    private initialMount(): void {
      ...
      if (this.getDisposeResult("ssr")) {
        this.ssrUrl = CompletionPath(
          globalEnv.rawWindow.location.pathname,
          this.appUrl
        );
      } else if (this.ssrUrl) {
        this.ssrUrl = "";
      ...
      if (appInstanceMap.has(this.appName)) {
        ...
        if (
          ...
          (app.isPrefetch || app.getAppState() === appStates.UNMOUNT)
        ) {
          ...
          logWarn(
            `the ${
              app.isPrefetch ? "prefetch" : "unmounted"
            } app with url: ${existAppUrl} is replaced by a new app`,
            this.appName
          );
          this.handleCreateApp();
        } else {
          logError(
            `app name conflict, an app named ${this.appName} is running`,
            this.appName
          );
        }
      } else {
    ...
    private handleAttributeUpdate = (): void => {
      ...
      const formatAttrUrl = formatAppURL(
        this.getAttribute("url"),
        this.appName
      );
      if (
        this.legalAttribute("name", formatAttrName) &&
        this.legalAttribute("url", formatAttrUrl)
      ) {
        ...
        if (formatAttrName !== this.appName && existApp) {
          ...
          if (
            ...
            keepAliveStates.KEEP_ALIVE_HIDDEN !==
              existApp.getKeepAliveState() &&
            !existApp.isPrefetch
          ) {
            this.setAttribute("name", this.appName);
            return logError(
              `app name conflict, an app named ${formatAttrName} is running`,
              this.appName
            );
          }
        }
        ...
        if (formatAttrName !== this.appName || formatAttrUrl !== this.appUrl) {
          if (formatAttrName === this.appName) {
            this.handleUnmount(true, () => {
              this.actionsForAttributeChange(
                formatAttrName,
                formatAttrUrl,
                existApp
              );
    ...
    private legalAttribute(name: string, val: AttrType): boolean {
      if (!isString(val) || !val) {
        logError(
          `unexpected attribute ${name}, please check again`,
          this.appName
        );
    ...
    private handleUnmount(
      ...
      const app = appInstanceMap.get(this.appName);
      if (app && app.getAppState() !== appStates.UNMOUNT)
    ...
    private getDisposeResult(name: string): boolean {
      // @ts-ignore
      return (
        (this.compatibleSpecialProperties(name) || microApp[name]) &&
        this.compatibleDisableSpecialProperties(name)
      );
    }
    ...
    private compatibleSpecialProperties(name: string): boolean {
      if (name === "disableScopecss") {
        return (
          this.hasAttribute("disableScopecss") ||
          this.hasAttribute("disable-scopecss")
        );
      } else if (name === "disableSandbox") {
        return (
          this.hasAttribute("disableSandbox") ||
          this.hasAttribute("disable-sandbox")
        );
      }
    ...
    private compatibleDisableSpecialProperties(name: string): boolean {
      if (name === "disableScopecss") {
        return (
          this.getAttribute("disableScopecss") !== "false" &&
          this.getAttribute("disable-scopecss") !== "false"
        );
      } else if (name === "disableSandbox") {
        return (
          this.getAttribute("disableSandbox") !== "false" &&
          this.getAttribute("disable-sandbox") !== "false"
        );
      }
    ...
    private getBaseRouteCompatible(): string {
      return (
        this.getAttribute("baseroute") ?? this.getAttribute("baseurl") ?? ""
      );
    }
    ...
    private getKeepAliveModeResult(): boolean {
      return (
        this.getDisposeResult("keep-alive") &&
