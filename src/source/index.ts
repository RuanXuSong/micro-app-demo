  ...
  for (const dom of children) {
    if (dom instanceof HTMLLinkElement) {
      if (dom.hasAttribute("exclude")) {
        ...
        dom.setAttribute(
          "href",
          CompletionPath(dom.getAttribute("href")!, app.url)
        );
      }
      ...
      dom instanceof HTMLMetaElement ||
      dom instanceof HTMLTitleElement
    ) {
      parent.removeChild(dom);
    } else if (dom instanceof HTMLImageElement && dom.hasAttribute("src")) {
      dom.setAttribute(
        "src",
        CompletionPath(dom.getAttribute("src")!, app.url)
      );
    }
  }
}
...
export default function extractHtml(app: AppInterface): void {
  fetchSource(app.ssrUrl || app.url, app.name, { cache: "no-cache" })
    .then((htmlStr: string) => {
      if (!htmlStr) {
    ...
    .catch((e) => {
      logError(
        `Failed to fetch data from ${app.url}, micro-app stop rendering`,
        app.name,
        e
      );
      app.onLoadError(e);
