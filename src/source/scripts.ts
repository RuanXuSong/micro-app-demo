...
export function extractScriptElement(
  ...
  if (script.hasAttribute("exclude")) {
    replaceComment = document.createComment(
      "script element with exclude attribute removed by micro-app"
    );
  } else if (
    (script.type &&
      ![
    ...
    if (!isDynamic) {
      ...
      replaceComment = document.createComment(
        `script with src='${src}' extract by micro-app`
      );
    } else {
      return { url: src, info };
    }
    ...
    if (!isDynamic) {
      ...
      replaceComment = document.createComment(
        "inline script extract by micro-app"
      );
    } else {
     ...
     */
    replaceComment = document.createComment(
      "script element removed by micro-app"
    );
  }
...
export function fetchScriptsFromHtml(
  ...
  const scriptEntries: Array<[string, sourceScriptInfo]> = Array.from(
    app.source.scripts.entries()
  );
...
export function execScripts(
  ...
  const scriptListEntries: Array<[string, sourceScriptInfo]> = Array.from(
    scriptList.entries()
  );
  ...
  for (const [url, info] of scriptListEntries) {
    if (!info.isDynamic) {
      ...
      if (info.defer || info.async) {
        ...
        info.module &&
          (initHook.moduleCount = initHook.moduleCount
            ? ++initHook.moduleCount
            : 1);
      } else {
...
export function runDynamicRemoteScript(
  ...
  if (app.inline || info.module) {
    ...
    replaceElement = document.createComment(
      `dynamic script with src='${url}' extract by micro-app`
    );
  }
  ...
  fetchSource(url, app.name)
    .then((code: string) => {
      ...
      try {
        ...
        console.error(
          `[micro-app from runDynamicScript] app ${app.name}: `,
          e,
          url
        );
      }
...
function runCode2InlineScript(
  ...
  if (module) {
    ...
    if (callback) {
      ...
      scriptElement.onload = callback.bind(
        scriptElement,
        callback.moduleCount === 0
      );
    }
  } else {
