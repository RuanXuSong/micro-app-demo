...
class CSSParser {
  ...
  private mediaRule = this.createMatcherForAtRuleWithChildRule(
    /^@media *([^{]+)/,
    "media"
  );
  // https://developer.mozilla.org/en-US/docs/Web/API/CSSSupportsRule
  private supportsRule = this.createMatcherForAtRuleWithChildRule(
    /^@supports *([^{]+)/,
    "supports"
  );
  private documentRule = this.createMatcherForAtRuleWithChildRule(
    /^@([-\w]+)?document *([^{]+)/,
    "document"
  );
  private hostRule = this.createMatcherForAtRuleWithChildRule(
    /^@host\s*/,
    "host"
  );
  ...
  private createMatcherForAtRuleWithChildRule(
    reg: RegExp,
    name: string
  ): () => boolean | void {
    return () => {
