...
function handleNewNode(parent: Node, child: Node, app: AppInterface): Node {
  if (child instanceof HTMLStyleElement) {
    if (child.hasAttribute("exclude")) {
      const replaceComment = document.createComment(
        "style element with exclude attribute ignored by micro-app"
      );
    ...
    const { replaceComment, url, info } =
      extractScriptElement(child, parent, app, true) || {};
...
function invokePrototypeMethod(
  ...
  if (container) {
    ...
    if (passiveChild && !container.contains(passiveChild)) {
      ...
      !container.contains(targetChild)
    ) {
...
export function patchElementPrototypeMethods(): void {
  ...
  Element.prototype.removeChild = function removeChild<T extends Node>(
    oldChild: T
  ): T {
  ...
  Element.prototype.cloneNode = function cloneNode(deep?: boolean): Node {
    ...
    this.__MICRO_APP_NAME__ &&
      (clonedNode.__MICRO_APP_NAME__ = this.__MICRO_APP_NAME__);
    return clonedNode;
  };
...
function patchDocument() {
  ...
  Document.prototype.createElementNS = function createElementNS(
    ...
    const element = globalEnv.rawCreateElementNS.call(
      this,
      namespaceURI,
      name,
      options
  ...
  function querySelectorAll(this: Document, selectors: string): any {
    ...
    return (
      appInstanceMap.get(appName)?.container?.querySelectorAll(selectors) ?? []
    );
  }
...
export function patchSetAttribute(): void {
  ...
  Element.prototype.setAttribute = function setAttribute(
    ...
    if (/^micro-app(-\S+)?/i.test(this.tagName) && key === "data") {
      ...
      const app = appInstanceMap.get(this.__MICRO_APP_NAME__);
      globalEnv.rawSetAttribute.call(
        this,
        key,
...
function releasePatchDocument(): void {
  ...
  Document.prototype.createDocumentFragment =
    globalEnv.rawCreateDocumentFragment;
  ...
  Document.prototype.getElementsByClassName =
    globalEnv.rawGetElementsByClassName;
